...
func (s *mserver) routes() {
	...
	s.path("/drive/upload").Methods("PUT", "POST").HandlerFunc(s.authOnly(s.handleAddDrivePhotos))
	s.path("/drive/job/schedule").Methods("PUT", "POST").HandlerFunc(s.authOnly(s.handleScheduleDriveJob))
	s.path("/drive/job/{id}").Methods("GET").HandlerFunc(s.authOnly(s.handleStatusDriveJob))

	s.path("/local/upload").Methods("POST", "PUT").HandlerFunc(s.authOnly(s.handleUploadLocalPhoto))
	s.path("/local/check").Methods("POST").HandlerFunc(s.authOnly(s.handleCheckLocalPhotos))
	...
	s.path("/comments/{img}").Methods("POST", "PUT").HandlerFunc(s.guestOnly(s.handleCommentPhoto))
	s.path("/comments/{img}").Methods("GET").HandlerFunc(s.loginInfo(s.handlePhotoComments))
	...
	s.path("/guest/likes/{img}").Methods("GET").HandlerFunc(s.guestOnly(s.handleGuestLikePhoto))
	s.path("/guest/verify").HandlerFunc(s.mResponse(s.handleVerifyGuest))
	s.path("/likes/{img}").Methods("POST", "PUT").HandlerFunc(s.guestOnly(s.handleLikePhoto))
	s.path("/likes/{img}").Methods("DELETE").HandlerFunc(s.guestOnly(s.handleUnlikePhoto))
	s.path("/likes/{img}").Methods("GET").HandlerFunc(s.loginInfo(s.handlePhotoLikes))
