...
func (s *mserver) routes() {
	...
	s.mDELETE("/guests/{guestid}", s.authOnly(s.handleDeleteGuest))
	s.mPUT("/guest", s.mResponse(s.handleCreateGuest))
	s.mGET("/guest", s.guestOnly(s.handleGuest))
	s.mDELETE("/guest", s.mResponse(s.handleDeleteGuestSelf))
	s.mPUT("/guest/update", s.guestOnly(s.handleUpdateGuest))
	s.mPUT("/guest/avatar/upload", s.guestOnly(s.uploadGuestAvatarFromFile))
	s.mPUT("/guest/avatar", s.guestOnly(s.uploadGuestAvatarFromURL))
	s.mDELETE("/guest/avatar", s.guestOnly(s.deleteGuestAvatar))
	s.mGET("/guest/avatar/{guestid}/{size}", s.handleGuestAvatar)
	s.mGET("/guest/avatar/{guestid}", s.handleGuestAvatar)
